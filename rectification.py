...
def vis_2_model(image, model_1, model_2, show=True):
    """Helper function to visualize computed model."""
    import matplotlib.pyplot as plt
    edgelets = compute_edgelets(image)
    locations_1, directions_1, strengths_1 = edgelets
    inliers_1 = compute_votes(edgelets, model_1, 10) > 0

    edgelets_1 = (locations_1[inliers_1], directions_1[inliers_1], strengths_1[inliers_1])
    locations_1, directions_1, strengths_1 = edgelets_1
    vis_edgelets(image, edgelets, False)
    vp_1 = model_1 / model_1[2]
    plt.plot(vp_1[0], vp_1[1], 'bo')
    for i in range(locations_1.shape[0]):
        xax = [locations_1[i, 0], vp_1[0]]
        yax = [locations_1[i, 1], vp_1[1]]
        plt.plot(xax, yax, 'b-.')

    locations_2, directions_2, strengths_2 = edgelets
    inliers_2 = compute_votes(edgelets, model_2, 10) > 0

    edgelets_2 = (locations_2[inliers_2], directions_2[inliers_2], strengths_2[inliers_2])
    locations_2, directions_2, strengths_2 = edgelets_2
    # vis_edgelets(image, edgelets, False)
    vp_2 = model_2 / model_2[2]
    plt.plot(vp_2[0], vp_2[1], 'bo')
    for i in range(locations_2.shape[0]):
        xax = [locations_2[i, 0], vp_2[0]]
        yax = [locations_2[i, 1], vp_2[1]]
        plt.plot(xax, yax, 'b-.')

    plt.plot([vp_1[0], vp_2[0]],[vp_1[1], vp_2[1]], color='red')
